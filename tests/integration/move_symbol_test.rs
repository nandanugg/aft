...
fn move_symbol_basic() {
    ...
    assert!(
        resp["checkpoint_name"].as_str().unwrap().contains("formatDate"),
    ...
    let consumer_a = std::fs::read_to_string(format!("{}/consumer_a.ts", root)).expect("read consumer_a");
    assert!(
...
fn move_symbol_multiple_consumers() {
    ...
    let cd_original = std::fs::read_to_string(fixture_path("move_symbol").join("consumer_d.ts")).unwrap();
    ...
    // Should: remain UNCHANGED
    let cf_original = std::fs::read_to_string(fixture_path("move_symbol").join("consumer_f.ts")).unwrap();
...
fn move_symbol_dry_run() {
    ...
    assert_eq!(source_before, source_after, "source should be unchanged after dry_run");
    assert_eq!(dest_before, dest_after, "dest should be unchanged after dry_run");
    assert_eq!(ca_before, ca_after, "consumer_a should be unchanged after dry_run");
...
fn move_symbol_non_top_level() {
    ...
    assert_eq!(resp["ok"], false, "should fail for class method: {:?}", resp);
    assert_eq!(
