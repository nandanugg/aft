...
fn edit_symbol_without_lsp_hints_returns_candidates() {
    ...
    assert_eq!(resp["code"], "ambiguous_symbol", "expected ambiguous, got: {:?}", resp);
    assert!(resp["candidates"].is_array(), "expected candidates array: {:?}", resp);
    assert!(resp["candidates"].as_array().unwrap().len() >= 2, "expected >= 2 candidates");
...
fn edit_symbol_with_malformed_lsp_hints_falls_back() {
    ...
    assert_eq!(resp["code"], "ambiguous_symbol", "expected ambiguous fallback, got: {:?}", resp);

    let (status, stderr) = aft.stderr_output();
    assert!(status.success());
    assert!(stderr.contains("ignoring malformed data"), "expected malformed warning in stderr, got: {}", stderr);
}
