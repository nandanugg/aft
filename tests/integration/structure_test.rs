...
fn wrap_try_catch_simple_function() {
    ...
    let content = fs::read_to_string(&tmp).unwrap();
    assert!(content.contains("try {"), "Expected try block in content:\n{}", content);
    assert!(content.contains("catch (error)"), "Expected catch block");
    assert!(content.contains("throw error;"), "Expected default catch body");
    // Original body should still be present (re-indented)
    assert!(content.contains("results.push(item.toUpperCase())"), "Body should be preserved");
    aft.shutdown();
}
...
fn wrap_try_catch_method_in_class() {
    ...
    let resp = send_wrap_try_catch(&mut aft, "w2", file, "fetch", Some("console.error(error);\nthrow error;"));
    assert_eq!(resp["ok"], true, "response: {:?}", resp);
    assert_eq!(resp["syntax_valid"], true);

    let content = fs::read_to_string(&tmp).unwrap();
    assert!(content.contains("try {"), "Expected try block");
    assert!(content.contains("console.error(error);"), "Expected custom catch body");
    aft.shutdown();
}
...
fn add_decorator_to_decorated_function_first() {
    ...
    let resp = send_add_decorator(&mut aft, "dec2", file, "get_users", "login_required", Some("first"));
    assert_eq!(resp["ok"], true, "response: {:?}", resp);
    assert_eq!(resp["syntax_valid"], true);
...
fn add_struct_tags_update_existing_value() {
    ...
    let resp = send_add_struct_tags(&mut aft, "t3", file, "User", "Email", "json", "email_addr,omitempty");
    assert_eq!(resp["ok"], true, "response: {:?}", resp);
