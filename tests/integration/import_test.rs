...
fn add_import_ts_external_group() {
    ...
    let resp = send_add_import(
        &mut aft, "imp-1", &file_str, "lodash",
        Some(&["debounce"]), None, false,
    );
    ...
    assert_eq!(resp["syntax_valid"], true, "syntax should be valid after add, resp: {:?}", resp);
...
fn add_import_ts_relative_group() {
    ...
    let resp = send_add_import(
        &mut aft, "imp-2", &file_str, "./components",
        Some(&["Button"]), None, false,
    );
    ...
    let button_pos = content.find("import { Button } from './components'").unwrap();
...
fn add_import_ts_dedup() {
    ...
    let resp = send_add_import(
        &mut aft, "imp-3", &file_str, "react",
        Some(&["useState"]), None, false,
    );
    ...
    assert_eq!(original, current, "file should not have been modified for duplicate");
...
fn add_import_ts_alphabetizes() {
    ...
    let resp = send_add_import(
        &mut aft, "imp-4", &file_str, "axios",
        None, Some("axios"), false,
    );
...
fn add_import_js_works() {
    ...
    let resp = send_add_import(
        &mut aft, "imp-5", &file_str, "cors",
        None, Some("cors"), false,
    );

    assert_eq!(resp["ok"], true, "add_import on JS should succeed: {:?}", resp);
    assert_eq!(resp["added"], true);
    assert_eq!(resp["group"], "external");
...
fn add_import_empty_file() {
    ...
    let resp = send_add_import(
        &mut aft, "imp-6", &file_str, "react",
        Some(&["useState"]), None, false,
    );

    assert_eq!(resp["ok"], true, "add_import on empty file should succeed: {:?}", resp);
    assert_eq!(resp["added"], true);
...
fn add_import_missing_file_returns_error() {
    ...
    let resp = send_add_import(
        &mut aft, "imp-7", "/tmp/nonexistent_aft_test.ts", "react",
        Some(&["useState"]), None, false,
    );
...
fn add_import_py_stdlib_group() {
    ...
    let resp = send_add_import(
        &mut aft, "py-1", &file_str, "pathlib",
        Some(&["Path"]), None, false,
    );

    assert_eq!(resp["ok"], true, "add_import py stdlib should succeed: {:?}", resp);
    assert_eq!(resp["added"], true);
...
fn add_import_py_third_party_group() {
    ...
    let resp = send_add_import(
        &mut aft, "py-2", &file_str, "click",
        None, None, false,
    );

    assert_eq!(resp["ok"], true, "add_import py third-party should succeed: {:?}", resp);
    assert_eq!(resp["added"], true);
    assert_eq!(resp["group"], "external");
    ...
    assert!(click_pos > os_pos, "third-party import should be after stdlib");
    assert!(click_pos < utils_pos, "third-party import should be before local");
...
fn add_import_py_local_group() {
    ...
    let resp = send_add_import(
        &mut aft, "py-3", &file_str, ".models",
        Some(&["User"]), None, false,
    );

    assert_eq!(resp["ok"], true, "add_import py local should succeed: {:?}", resp);
    assert_eq!(resp["added"], true);
    assert_eq!(resp["group"], "internal");
    ...
    let requests_pos = content.find("import requests").unwrap();
    assert!(models_pos > requests_pos, "local import should be after third-party");
...
fn add_import_rs_std_group() {
    ...
    let resp = send_add_import(
        &mut aft, "rs-1", &file_str, "std::fmt::Display",
        None, None, false,
    );

    assert_eq!(resp["ok"], true, "add_import rs std should succeed: {:?}", resp);
    assert_eq!(resp["added"], true);
    assert_eq!(resp["group"], "stdlib");
    ...
    assert!(fmt_pos < serde_pos, "std import should be before external imports");
...
fn add_import_rs_external_group() {
    ...
    let resp = send_add_import(
        &mut aft, "rs-2", &file_str, "anyhow::Result",
        None, None, false,
    );

    assert_eq!(resp["ok"], true, "add_import rs external should succeed: {:?}", resp);
    assert_eq!(resp["added"], true);
    assert_eq!(resp["group"], "external");
    ...
    assert!(anyhow_pos < crate_pos, "external import should be before crate");
...
fn add_import_rs_dedup() {
    ...
    let resp = send_add_import(
        &mut aft, "rs-3", &file_str, "std::collections::HashMap",
        None, None, false,
    );
...
fn add_import_go_stdlib_group() {
    ...
    let resp = send_add_import(
        &mut aft, "go-1", &file_str, "net/http",
        None, None, false,
    );

    assert_eq!(resp["ok"], true, "add_import go stdlib should succeed: {:?}", resp);
    assert_eq!(resp["added"], true);
    assert_eq!(resp["group"], "stdlib");
...
fn add_import_go_external_group() {
    ...
    let resp = send_add_import(
        &mut aft, "go-2", &file_str, "golang.org/x/tools",
        None, None, false,
    );

    assert_eq!(resp["ok"], true, "add_import go external should succeed: {:?}", resp);
    assert_eq!(resp["added"], true);
    assert_eq!(resp["group"], "external");
...
fn remove_import_entire_statement_ts() {
    ...
    assert!(content.contains("from 'react'"), "react imports should remain");
...
",
    ...
    assert_eq!(resp["ok"], true, "organize_imports should succeed: {:?}", resp);
...
",
    ...
    assert_eq!(zod_count, 1, "should have exactly one zod import. content:\n{}", content);

    let react_count = content.matches("from 'react'").count();
    assert_eq!(react_count, 1, "should have exactly one react import. content:\n{}", content);
...
",
    ...
    assert!(groups.len() >= 2, "should have at least 2 groups: {:?}", groups);
