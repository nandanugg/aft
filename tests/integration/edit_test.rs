...
fn write_creates_new_file() {
    ...
    assert_eq!(resp["created"], true, "file was new, created should be true");
    // No backup_id for new files
    assert!(resp.get("backup_id").is_none() || resp["backup_id"].is_null(),
...
fn write_backups_existing_file() {
    ...
    assert!(resp["backup_id"].is_string(), "should have backup_id for existing file");
    ...
    assert_eq!(undo_resp["ok"], true, "undo should succeed: {:?}", undo_resp);
...
fn write_syntax_valid() {
    ...
    assert_eq!(resp["ok"], true, "write should succeed: {:?}", resp);
    assert_eq!(resp["syntax_valid"], true, "valid TS should have syntax_valid: true");
...
fn write_syntax_invalid() {
    ...
    assert_eq!(resp["ok"], true, "write should succeed even with bad syntax: {:?}", resp);
    assert_eq!(resp["syntax_valid"], false, "broken TS should have syntax_valid: false");
...
fn edit_symbol_replace() {
    ...
    assert_eq!(resp["ok"], true, "edit_symbol replace should succeed: {:?}", resp);
    ...
    assert!(content.contains("Hey,"), "should contain new greeting: {}", content);
    assert!(!content.contains("Hello,"), "should not contain old greeting");
...
fn edit_symbol_delete() {
    ...
    assert_eq!(resp["ok"], true, "edit_symbol delete should succeed: {:?}", resp);
    assert_eq!(resp["operation"], "delete");

    // Verify symbol is gone from disk
    let after = fs::read_to_string(&target).unwrap();
    assert!(!after.contains("function internalHelper"), "internalHelper should be deleted");
...
fn edit_symbol_ambiguous() {
    ...
    assert_eq!(resp["ok"], true, "ambiguous response should succeed: {:?}", resp);
    assert_eq!(resp["code"], "ambiguous_symbol");
    let candidates = resp["candidates"].as_array().expect("should have candidates array");
    assert!(candidates.len() >= 2, "should have at least 2 candidates: {:?}", candidates);

    // Each candidate should have name, qualified, line, kind
    for c in candidates {
        assert!(c["name"].is_string(), "candidate should have name: {:?}", c);
        assert!(c["qualified"].is_string(), "candidate should have qualified: {:?}", c);
...
fn edit_symbol_not_found() {
    ...
    assert_eq!(resp["ok"], false, "should fail for nonexistent symbol: {:?}", resp);
    assert_eq!(resp["code"], "symbol_not_found");
    assert!(resp["message"].as_str().unwrap().contains("nonexistent_symbol"));
...
fn edit_match_single_occurrence() {
    ...
    // Verify content on disk
    let content = fs::read_to_string(&target).unwrap();
    assert!(content.contains("secretHelper"), "should contain replacement");
    assert!(!content.contains("internalHelper"), "should not contain original match");
...
fn edit_match_multiple_occurrences_returns_candidates() {
    ...
    let occurrences = resp["occurrences"].as_array().expect("should have occurrences array");
    ...
    assert_eq!(fs::read_to_string(&target).unwrap(), content, "file should not be modified");
...
fn edit_match_with_occurrence_selector() {
    ...
    assert_eq!(resp["ok"], true, "edit_match with occurrence should succeed: {:?}", resp);
    assert_eq!(resp["replacements"], 1);

    // Verify: first "hello" untouched, second replaced
    let result = fs::read_to_string(&target).unwrap();
    assert!(result.contains("const a = \"hello\""), "first occurrence should be untouched");
    assert!(result.contains("const b = \"bye\""), "second occurrence should be replaced");
...
fn batch_multiple_edits() {
    ...
    let result = fs::read_to_string(&target).unwrap();
    assert!(result.contains("\"hi\""), "first edit should apply: {}", result);
    assert!(result.contains("\"bye\""), "second edit should apply: {}", result);
    assert!(result.contains("count = 42"), "untouched line should remain");
...
fn batch_rollback_on_failure() {
    ...
    let on_disk = fs::read_to_string(&target).unwrap();
    assert_eq!(on_disk, content, "file should be unchanged after failed batch");
...
fn batch_line_range_edit() {
    ...
    assert_eq!(resp["ok"], true, "batch line-range should succeed: {:?}", resp);
    assert_eq!(resp["edits_applied"], 1);

    let result = fs::read_to_string(&target).unwrap();
    assert!(result.contains("line zero"), "line 0 untouched");
    assert!(result.contains("replaced line"), "line 1 should be replaced");
    assert!(!result.contains("line one"), "original line 1 should be gone");
