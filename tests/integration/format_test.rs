...
fn format_test_dir() -> std::path::PathBuf {
    let dir = std::env::temp_dir().join("aft_format_tests");
...
/// Write a poorly-formatted .rs file via `write` command → verify rustfmt runs.
#[test]
fn format_integration_applied_rustfmt() {
    ...
    assert_eq!(resp["formatted"], true, "rustfmt should have formatted the file");
    assert!(
...
fn format_integration_unsupported_language() {
    ...
    assert_eq!(resp["ok"], true, "write should succeed: {:?}", resp);
    assert_eq!(resp["formatted"], false, "txt files should not be formatted");
    assert_eq!(
...
fn format_integration_not_found() {
    ...
    assert_eq!(resp["ok"], true, "write should succeed: {:?}", resp);
    assert_eq!(resp["formatted"], false, "should not be formatted without formatter");
    assert_eq!(
...
fn format_integration_fields_always_present() {
    ...
    assert_eq!(resp2["ok"], true, "write to .rs should succeed: {:?}", resp2);
    assert!(
...
fn validate_full_default_no_errors() {
    ...
    let has_errors = resp.get("validation_errors").is_some()
        ...
        && resp["validation_errors"].as_array().map_or(false, |a| !a.is_empty());
    assert!(
        !has_errors,
...
fn validate_full_with_checker() {
    ...
    let has_validation = resp.get("validation_errors").is_some()
        || resp.get("validate_skipped_reason").is_some();
    assert!(
        has_validation,
