            ...
            if cursor.goto_first_child() {
                loop {
                    ...
                    if node.kind() == "class_declaration" {
                        if let Some(name) = find_child_text(&node, source, &["type_identifier", "identifier"]) {
                        ...
                        if inner.goto_first_child() {
                            loop {
                                ...
                                if child.kind() == "class_declaration" {
                                    if let Some(name) = find_child_text(&child, source, &["type_identifier", "identifier"]) {
                ...
                if cursor.goto_first_child() {
                    loop {
                        ...
                        if child.kind() == "decorated_definition" {
                            if let Some(result) = walk_py_classes(&child, source, scope_name, available) {
                                return Some(result);
                            }
                        }
                        if !cursor.goto_next_sibling() {
                        ...
                        if let Some(name) = find_child_text(&node, source, &["type_identifier"]) {
                            ...
                            if name == scope_name && struct_match.is_none() {
                                if let Some(body) = find_child_by_kind(&node, "field_declaration_list") {
                                    ...
                                    struct_match = Some((body.start_byte(), body.end_byte(), children));
                                }
                            }
                        }
                    }
            ...
            if cursor.goto_first_child() {
                loop {
                    ...
                    if node.kind() == "type_declaration" {
                        if let Some(info) = find_go_struct(&node, source, scope_name, &mut available) {
                            return (Some(info), available);
                        }
                    }
                    if !cursor.goto_next_sibling() {
                ...
                if let Some(name_node) = find_child_by_kind(&child, "type_identifier") {
                    ...
                    if name == scope_name {
                        if let Some(struct_type) = find_child_by_kind(&child, "struct_type") {
                            if let Some(body) = find_child_by_kind(&struct_type, "field_declaration_list") {
...
fn resolve_position(
    ...
    match position {
        "first" => {
            Ok(resolve_first(source, body_start, lang))
        }
        "last" => {
            Ok(resolve_last(source, body_end, children, lang))
        }
