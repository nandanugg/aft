...
pub fn handle_add_derive(req: &RawRequest, ctx: &AppContext) -> Response {
    ...
    let (new_source, final_derives) =
        apply_derive(&source, &root, target_info, &derives);
    ...
    if let Err(e) = std::fs::write(path, &new_source) {
        return Response::error(
            &req.id,
            "invalid_request",
            format!("add_derive: failed to write file: {}", e),
        );
    }
    ...
    match derive_attr {
        ...
        None => {
            ...
            let derive_line = format!(
                "{}#[derive({})]\n",
                indent,
                new_derives.iter().map(|s| s.as_str()).collect::<Vec<_>>().join(", ")
            );
