...
pub fn handle_add_struct_tags(req: &RawRequest, ctx: &AppContext) -> Response {
    ...
    let (new_source, final_tag_string) = match field_info.existing_tag {
        Some(tag_info) => {
            ...
            let new_source = edit::replace_byte_range(&source, tag_info.start, tag_info.end, &new_tag_str);
        ...
        None => {
            ...
            let new_source = edit::replace_byte_range(&source, insert_pos, insert_pos, &insert_text);
            (new_source, tag_str)
        }
    };
            ...
            if child.kind() == "field_declaration" {
                if let Some(fi_node) = find_child_by_kind_node(&child, "field_identifier") {
                    ...
                    if fname == field_name {
                        ...
                        let tag = find_child_by_kind_node(&child, "raw_string_literal")
                            .map(|n| TagRange {
                                start: n.start_byte(),
                                end: n.end_byte(),
                            });
                    ...
                    loop {
                        ...
                        if child.kind() == "type_spec" {
                                ...
                                find_child_by_kind_node(&child, "type_identifier")
                            {
                                ...
                                if name == struct_name {
                                    if let Some(st) =
                                        find_child_by_kind_node(&child, "struct_type")
                                    {
                                        if let Some(body) = find_child_by_kind_node(
                                            &st,
