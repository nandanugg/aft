...
pub fn handle_add_import(req: &RawRequest, ctx: &AppContext) -> Response {
    ...
    if !imports::is_supported(lang) {
        return Response::error(
            &req.id,
            "invalid_request",
            format!("add_import: import management not yet supported for {:?}", lang),
        );
    }
    ...
    // --- Auto-backup ---
    let backup_id = match edit::auto_backup(ctx, path, "add_import: pre-edit backup") {
        Ok(id) => id,
        Err(e) => {
            return Response::error(&req.id, e.code(), e.to_string());
        }
    ...
    if let Err(e) = std::fs::write(path, &new_source) {
        return Response::error(
            &req.id,
            "invalid_request",
            format!("add_import: failed to write file: {}", e),
        );
    }
