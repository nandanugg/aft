...
pub fn handle_add_decorator(req: &RawRequest, ctx: &AppContext) -> Response {
    ...
    // --- Auto-backup ---
    let backup_id = match edit::auto_backup(ctx, path, "add_decorator: pre-edit backup") {
        Ok(id) => id,
        Err(e) => {
    ...
    // --- Insert ---
    let new_source = edit::replace_byte_range(&source, insert_offset, insert_offset, &decorator_line);
    ...
    if let Err(e) = std::fs::write(path, &new_source) {
        return Response::error(
            &req.id,
            "invalid_request",
            format!("add_decorator: failed to write file: {}", e),
        );
    }
