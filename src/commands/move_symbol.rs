...
pub fn handle_move_symbol(req: &RawRequest, ctx: &AppContext) -> Response {
    ...
    let source_canon = std::fs::canonicalize(source_path_raw).unwrap_or_else(|_| source_path_raw.to_path_buf());
    ...
    let end_byte = edit::line_col_to_byte(
        &source_content,
...
pub fn compute_relative_import_path(from_file: &Path, to_file: &Path) -> String {
    ...
    let to_stem = to_file.file_stem().unwrap_or_default().to_str().unwrap_or("");
...
fn compute_relative_dir(from: &Path, to: &Path) -> String {
    ...
    let from_parts: Vec<&str> = from.components()
        .filter_map(|c| match c {
            std::path::Component::Normal(s) => s.to_str(),
            _ => None,
        })
        .collect();
    let to_parts: Vec<&str> = to.components()
        .filter_map(|c| match c {
            std::path::Component::Normal(s) => s.to_str(),
            _ => None,
        })
...
fn generate_import_with_alias(
    ...
    format!("import {}{{ {} }} from '{}';", type_prefix, names_str, new_module_path)
}
...
fn restore_checkpoint(ctx: &AppContext, name: &str) {
    ...
    if let Err(e) = cp_store.restore(name) {
        eprintln!("[aft] move_symbol rollback: failed to restore checkpoint '{}': {}", name, e);
    }
}
