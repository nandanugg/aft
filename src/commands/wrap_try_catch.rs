...
pub fn handle_wrap_try_catch(req: &RawRequest, ctx: &AppContext) -> Response {
    ...
    // --- Auto-backup ---
    let backup_id = match edit::auto_backup(ctx, path, "wrap_try_catch: pre-edit backup") {
        Ok(id) => id,
        Err(e) => {
    ...
    if let Err(e) = std::fs::write(path, &new_source) {
        return Response::error(
            &req.id,
            "invalid_request",
            format!("wrap_try_catch: failed to write file: {}", e),
        );
    }
        ...
        loop {
            ...
            match child.kind() {
                ...
                _ => {
                    ...
                    if let Some(info) =
                        walk_for_function(&child, source, target_name, available)
                    {
                        return Some(info);
                    }
                }
