...
pub fn is_duplicate(
    ...
    for imp in &block.imports {
        ...
        if names.is_empty() && default_import.is_none()
            && imp.names.is_empty() && imp.default_import.is_none()
        {
            return true;
        }
...
pub fn parse_file_imports(
    ...
    let source = std::fs::read_to_string(path).map_err(|e| crate::error::AftError::FileNotFound {
    ...
    parser.set_language(&grammar).map_err(|e| {
        crate::error::AftError::ParseError {
...
fn parse_single_ts_import(source: &str, node: &Node) -> Option<ImportStatement> {
    ...
    if child_cursor.goto_first_child() {
        loop {
            ...
            match child.kind() {
                "import_clause" => {
                    extract_import_clause(source, &child, &mut names, &mut default_import, &mut namespace_import);
                }
        ...
        if child.kind() == "import_specifier" {
            ...
            if let Some(name_node) = child.child_by_field_name("name") {
                ...
                if spec_cursor.goto_first_child() {
                    loop {
                        if spec_cursor.node().kind() == "identifier" || spec_cursor.node().kind() == "type_identifier" {
...
const PYTHON_STDLIB: &[&str] = &[
    ...
    "tabnanny", "tarfile", "tempfile", "termios", "textwrap", "threading",
...
fn parse_py_imports(source: &str, tree: &Tree) -> ImportBlock {
    ...
    ImportBlock { imports, byte_range }
}
        ...
        loop {
            ...
            match child.kind() {
                "dotted_name" => {
                    ...
                    if module_path.is_empty() && !has_seen_import_keyword(source, node, child.start_byte()) {
...
fn parse_rs_use_declaration(source: &str, node: &Node) -> Option<ImportStatement> {
    ...
    if c.goto_first_child() {
        loop {
            let child = c.node();
            match child.kind() {
                "visibility_modifier" => { has_pub = true; }
    ...
    if use_path.is_empty() { return None; }
    ...
    Some(ImportStatement {
        ...
        names,
        default_import: if has_pub { Some("pub".to_string()) } else { None },
        namespace_import: None,
        kind: ImportKind::Value,
        group,
        byte_range,
...
fn parse_go_import_spec(source: &str, node: &Node) -> Option<ImportStatement> {
    ...
    if import_path.is_empty() { return None; }
...
pub fn generate_go_import_line_pub(module_path: &str, alias: Option<&str>, in_group: bool) -> String {
...
";
        ...
        let external: Vec<_> = block.imports.iter().filter(|i| i.group == ImportGroup::External).collect();
        let relative: Vec<_> = block.imports.iter().filter(|i| i.group == ImportGroup::Internal).collect();
    ...
    fn dedup_detects_same_named_import() {
        ...
        let (_, block) = parse_ts(source);
        assert!(is_duplicate(&block, "react", &["useState".to_string()], None, false));
    }
    ...
    fn dedup_misses_different_name() {
        let source = "import { useState } from 'react';\n";
        let (_, block) = parse_ts(source);
        assert!(!is_duplicate(&block, "react", &["useEffect".to_string()], None, false));
    }
    ...
    fn dedup_type_vs_value() {
        ...
        assert!(!is_duplicate(&block, "react", &["FC".to_string()], None, true));
    }
    ...
    fn insertion_empty_file() {
        let source = "";
        let (_, block) = parse_ts(source);
        let (offset, _, _) = find_insertion_point(source, &block, ImportGroup::External, "react", false);
        assert_eq!(offset, 0);
    }
...
";
        let (_, block) = parse_ts(source);
        let (offset, _, _) = find_insertion_point(source, &block, ImportGroup::External, "bravo", false);
    ...
    fn parse_py_three_groups() {
        let source = "import os\nimport sys\n\nimport requests\n\nfrom . import utils\n";
        let (_, block) = parse_py(source);
        let stdlib: Vec<_> = block.imports.iter().filter(|i| i.group == ImportGroup::Stdlib).collect();
        let external: Vec<_> = block.imports.iter().filter(|i| i.group == ImportGroup::External).collect();
        let internal: Vec<_> = block.imports.iter().filter(|i| i.group == ImportGroup::Internal).collect();
    ...
    fn generate_py_from_import() {
        let line = generate_import_line(
            LangId::Python, "collections",
            &["OrderedDict".to_string()], None, false,
        );
    ...
    fn generate_py_from_import_multiple() {
        let line = generate_import_line(
            LangId::Python, "typing",
            &["Optional".to_string(), "List".to_string()], None, false,
        );
    ...
    fn classify_rs_groups() {
        assert_eq!(classify_group_rs("std::collections::HashMap"), ImportGroup::Stdlib);
        assert_eq!(classify_group_rs("core::mem"), ImportGroup::Stdlib);
        assert_eq!(classify_group_rs("alloc::vec"), ImportGroup::Stdlib);
        assert_eq!(classify_group_rs("serde::Deserialize"), ImportGroup::External);
    ...
    fn classify_go_groups() {
        ...
        assert_eq!(classify_group_go("github.com/pkg/errors"), ImportGroup::External);
        assert_eq!(classify_group_go("golang.org/x/tools"), ImportGroup::External);
    }
