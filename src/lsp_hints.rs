...
pub fn apply_lsp_disambiguation(
    ...
    let aligned_indices: Vec<usize> = matches
        ...
        .filter_map(|(i, m)| {
            ...
            if is_aligned { Some(i) } else { None }
        })
        .collect();
...
mod tests {
    ...
    fn make_match(name: &str, file: &str, start_line: u32, end_line: u32, kind: SymbolKind) -> SymbolMatch {
        SymbolMatch {
            symbol: Symbol {
                name: name.into(),
                kind,
    ...
    fn disambiguate_no_match_returns_all() {
        ...
        let result = apply_lsp_disambiguation(matches, &hints);
        assert_eq!(result.len(), 2, "no hint matches → fallback to all candidates");
    }
    ...
    fn disambiguate_stale_hint_ignored() {
        ...
        let result = apply_lsp_disambiguation(matches, &hints);
        assert_eq!(result.len(), 2, "stale hint should fall back to all candidates");
    }
    ...
    fn disambiguate_single_input_unchanged() {
        let matches = vec![
            make_match("foo", "bar.ts", 1, 5, SymbolKind::Function),
        ];
        let hints = LspHints {
            symbols: vec![LspSymbolHint {
                name: "foo".into(),
