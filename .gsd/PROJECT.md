...
**M001 (Foundation), M002 (Language Intelligence) complete. M003/S01 (Call Graph Infrastructure) and M003/S02 (Reverse Callers + File Watcher) complete.** The `aft` Rust binary runs as a persistent process with NDJSON protocol, embeds tree-sitter grammars for 6 languages (TypeScript, JavaScript, TSX, Python, Rust, Go), and handles 23 domain commands: the original 11 from M001, 3 import commands, 5 structure commands, `transaction`, and 3 call graph commands (`configure`, `call_tree`, `callers`). All 12 mutation commands support `dry_run: true` and auto-format. The call graph engine builds lazily from a configured project root, resolves cross-file calls through import chains, supports both forward call trees and reverse caller lookup with recursive depth expansion. A `notify` v8 file watcher invalidates stale graph data on file changes, proven by modify-then-query integration tests. 326 Rust tests + 39 plugin tests pass. Next: S03 (Trace to Entry Points).
