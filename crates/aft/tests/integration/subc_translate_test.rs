...
fn refactor_translate_matches_typescript_golden_fixtures() {
    ...
    assert!(
        cases.len() >= 7,
        "expected >=7 aft_refactor translate parity fixtures, found {}",
        cases.len()
    );
    ...
    assert!(
        failures.is_empty(),
        "{} aft_refactor translate parity mismatch(es):\n\n{}",
        failures.len(),
        failures.join("\n\n")
    );
}
...
fn safety_translate_matches_typescript_golden_fixtures() {
    ...
    assert!(
        cases.len() >= 8,
        "expected >=8 aft_safety translate parity fixtures, found {}",
        cases.len()
    );
    ...
    assert!(
        failures.is_empty(),
        "{} aft_safety translate parity mismatch(es):\n\n{}",
        failures.len(),
        failures.join("\n\n")
    );
}
