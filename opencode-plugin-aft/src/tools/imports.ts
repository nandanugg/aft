...
export function importTools(bridge: BinaryBridge): Record<string, ToolDefinition> {
  return {
    add_import: {
      description:
        "Add an import statement to a file. Automatically detects the correct group (stdlib/external/internal), inserts alphabetically within the group, and deduplicates. Supports TS, JS, TSX, Python, Rust, and Go.",
      args: {
    ...
    remove_import: {
      description:
        "Remove an import statement from a file, or remove a specific name from a multi-name import. If no name is specified, removes the entire import for the given module. Returns import_not_found error if the module is not imported.",
      args: {
    ...
    organize_imports: {
      description:
        "Organize all imports in a file: re-group by language convention (stdlib → external → internal), sort alphabetically within groups, deduplicate, and insert blank lines between groups. For Rust, merges common-prefix use declarations into use trees.",
      args: {
        file: z.string().describe("Path to the file whose imports should be organized"),
      },
      execute: async (args): Promise<string> => {
        const response = await bridge.send("organize_imports", {
          file: args.file,
        });
        return JSON.stringify(response);
      },
